...
function openModal() {
    ...
    // Esconder todas as notificações flutuantes
...
function closeModal() {
    ...
    // Esconder mensagens ao fechar
    hideFormMessages();

    // Mostrar novamente as notificações flutuantes após um delay
    setTimeout(() => {
        const notifications = document.querySelectorAll('.floating-notification');
        notifications.forEach(notif => {
            notif.style.display = '';
...
// Funções para mostrar/esconder mensagens do formulário
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Ajustar scroll quando input recebe foco (para manter botão visível quando teclado abre)
    const formInputs = document.querySelectorAll('#inscricaoForm input');
    const submitButton = document.querySelector('#inscricaoForm button[type="submit"]');
    const modalContent = document.querySelector('.modal-content');

    function scrollToShowButton() {
        if (submitButton && modalContent) {
            // Aguardar um pouco para o teclado aparecer e viewport ajustar
            setTimeout(() => {
                const buttonRect = submitButton.getBoundingClientRect();
                const viewportHeight = window.visualViewport ? window.visualViewport.height : window.innerHeight;
                const viewportTop = window.visualViewport ? window.visualViewport.offsetTop : 0;
                
                // Área visível considerando o teclado (aproximadamente 40% da altura)
                const visibleAreaBottom = viewportTop + viewportHeight;
                
                // Se o botão estiver abaixo da área visível
                if (buttonRect.bottom > visibleAreaBottom - 20) {
                    // Scroll para mostrar o botão com margem
                    const scrollAmount = buttonRect.bottom - (visibleAreaBottom - 20);
            ...
            }, 400); // Delay para o teclado aparecer completamente
        }
    }

    formInputs.forEach(input => {
        input.addEventListener('focus', scrollToShowButton);
        
        // Também ajustar quando o input recebe foco via toque
        input.addEventListener('touchstart', function() {
            setTimeout(scrollToShowButton, 100);
        });
    });

    // Ajustar quando o viewport muda (teclado abre/fecha)
    if (window.visualViewport) {
        window.visualViewport.addEventListener('resize', function() {
            const activeElement = document.activeElement;
            if (activeElement && activeElement.tagName === 'INPUT' && 
                activeElement.closest('#inscricaoForm')) {
                scrollToShowButton();
            }
        });
    }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (form) {
        form.addEventListener('submit', async function (e) {
            ...
            // Validação
            if (!nome || nome.length < 3) {
                showFormError('Por favor, insira seu nome completo.');
                submitButton.disabled = false;
                submitButton.innerHTML = originalText;
                return;
            }
            ...
            if (!telefone || telefone.length < 14) {
                showFormError('Por favor, insira um telefone válido com DDD.');
                submitButton.disabled = false;
                submitButton.innerHTML = originalText;
                return;
            }
            ...
            fetch('https://n8n.raiarruda.com.br/webhook/Rastreamento-de-dados', {
                method: 'POST',
                headers: {
                ...
                .then(async response => {
                    ...
                    return response.json().catch(() => {
                        // Se não for JSON, ainda considera sucesso se status for 200-299
                        return { success: true };
                    });
                })
                .then(data => {
                    console.log('Sucesso:', data);
                    showFormSuccess('🎉 Parabéns! Sua inscrição foi realizada com sucesso! Redirecionando...');
                    
                    // Capturar valores dos campos para adicionar à URL
                    const nomeValue = encodeURIComponent(nome || '');
                    const emailValue = encodeURIComponent(email || '');
                    const telefoneValue = encodeURIComponent(telefone || '');
                    const utmSource = encodeURIComponent(document.getElementById('utm_source')?.value || '');
                    const utmMedium = encodeURIComponent(document.getElementById('utm_medium')?.value || '');
                    const utmCampaign = encodeURIComponent(document.getElementById('utm_campaign')?.value || '');
                    const utmContent = encodeURIComponent(document.getElementById('utm_content')?.value || '');
                    const utmTerm = encodeURIComponent(document.getElementById('utm_term')?.value || '');
                    
                    // Construir o parâmetro sck (source-campaign-key)
                    const sck = `${utmSource}-${utmMedium}-${utmCampaign}-${utmContent}-${utmTerm}`;
                ...
                .catch((error) => {
                    ...
                    let errorMessage = 'Ops! Ocorreu um erro ao enviar seu formulário.';
                    
                    if (error.name === 'AbortError') {
                        errorMessage = 'O envio está demorando muito. Verifique sua conexão e tente novamente.';
                        ...
                        errorMessage = 'Erro de conexão. Verifique sua internet e tente novamente. O webhook pode estar temporariamente indisponível.';
                    } else if (error.message.includes('CORS') || error.message.includes('CORS policy')) {
                        errorMessage = 'Erro de configuração do servidor. Entre em contato conosco.';
                    } else if (error.message.includes('404') || error.message.includes('Not Found')) {
                        errorMessage = 'Serviço temporariamente indisponível. O webhook não foi encontrado. Tente novamente em alguns instantes.';
                    } else if (error.message.includes('500') || error.message.includes('Internal Server Error')) {
                        errorMessage = 'Erro no servidor. Tente novamente em alguns instantes.';
                    } else if (error.message.includes('403') || error.message.includes('Forbidden')) {
                        errorMessage = 'Acesso negado. Entre em contato conosco.';
                    }
                    
                    showFormError(errorMessage + ' Se o problema persistir, entre em contato conosco.');
                })
                .finally(() => {
